...
- `zetacnot0` (file: `StrongPNT/PNT3_RiemannZeta.lean`): added a spurious hypothesis `|t| > 2` even though the proof only uses `Re(3/2 + I*t) > 1`. Propagated this requirement into a few downstream lemmas/usages (notably in `StrongPNT/PNT4_ZeroFreeRegion.lean`) and stopped by replacing `zeta_c_nonzero` with `sorry`.
